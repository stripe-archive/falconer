...
CMD cp -r henson /build/ && env GOBIN=/build go install -a -v -ldflags "-X github.com/stripe/falconer.VERSION=$(git rev-parse HEAD) -X github.com/stripe/falconer.BUILD_DATE=$(date +%s)" ./cmd/...
